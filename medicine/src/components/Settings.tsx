            ...
            <div className="bg-white/90 backdrop-blur-sm shadow-xl border border-blue-100 rounded-3xl p-10 min-h-[600px]">
              ...
              {activeTab === 'notifications' && (
                <div className="space-y-8 animate-fade-in text-left">
                  <div>
                    <h2 className="text-2xl font-black text-gray-800 tracking-tight">Delivery Methods</h2>
                    <p className="text-gray-500 text-sm mt-1">Select how you want to be reached</p>
                  </div>

                  <div className="space-y-4">
                    {Object.entries(notifications).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between p-6 bg-white rounded-3xl border border-blue-100 hover:shadow-lg transition-all group shadow-sm">
                        <div className="flex items-center space-x-4">
                          <div className={`p-3 rounded-2xl ${value ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
              ...
              {activeTab === 'privacy' && (
                <div className="space-y-10 animate-fade-in text-left">
                  <div>
                    <h2 className="text-2xl font-black text-gray-800 tracking-tight">Data Governance</h2>
                    <p className="text-gray-500 text-sm mt-1">Control your data visibility and security protocols</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-8 bg-emerald-500/5 rounded-[2.5rem] relative overflow-hidden border border-emerald-100 group text-left">
                      <Shield className="text-emerald-500 mb-6 group-hover:scale-110 transition-transform duration-500" size={32} />
                      <h4 className="font-black text-gray-800 mb-2 uppercase tracking-tighter text-sm">Two-Factor Auth</h4>
                      <p className="text-xs text-gray-500 mb-6 leading-relaxed">Adds an extra layer of security to your health records.</p>
                      <button
                        onClick={() => setTwoFactorEnabled(!twoFactorEnabled)}
                        className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${twoFactorEnabled ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-white text-gray-600 shadow-md border border-gray-100'
                          }`}
                      >
                    ...
                    <div className="p-8 bg-blue-500/5 rounded-[2.5rem] relative overflow-hidden border border-blue-100 group text-left">
                      <Globe className="text-blue-500 mb-6 group-hover:scale-110 transition-transform duration-500" size={32} />
                      <h4 className="font-black text-gray-800 mb-2 uppercase tracking-tighter text-sm">Health Network</h4>
                      <p className="text-xs text-gray-500 mb-6 leading-relaxed">Anonymously share data for medical research advancement.</p>
                      <button
                        className="px-6 py-2.5 rounded-2xl bg-white text-blue-500 shadow-md border border-blue-50 text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-colors"
                      >
                        Manage Sharing
                      </button>
                    </div>

                    <div className="p-8 bg-purple-500/5 rounded-[2.5rem] relative overflow-hidden border border-purple-100 group text-left col-span-1 md:col-span-2">
                      <CreditCard className="text-purple-500 mb-6 group-hover:scale-110 transition-transform duration-500" size={32} />
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="font-black text-gray-800 mb-2 uppercase tracking-tighter text-sm">Payment Methods</h4>
                          <p className="text-xs text-gray-500 mb-4 leading-relaxed max-w-md">Securely manage your saved cards and billing details.</p>
                        </div>
                        <button
                          className="px-6 py-2.5 rounded-2xl bg-white text-purple-500 shadow-md border border-purple-50 text-[10px] font-black uppercase tracking-widest hover:bg-purple-50 transition-colors"
                        >
                          Manage Cards
                        </button>
                      </div>
                  ...
                  <div className="pt-10 border-t border-gray-100 space-y-4">
                    <button className="w-full flex items-center justify-between p-6 bg-white rounded-3xl hover:bg-gray-50 transition-colors group border border-blue-100 shadow-sm">
                      <div className="flex items-center space-x-4">
                        <div className="p-3 bg-gray-100 rounded-2xl shadow-sm"><Download size={20} className="text-gray-500" /></div>
                        <span className="font-bold text-gray-600 text-sm">Request Health Data Export</span>
                      </div>
                      <ChevronRight size={18} className="text-gray-400 group-hover:translate-x-1 transition-transform" />
                    </button>
                    <button className="w-full flex items-center justify-between p-6 bg-white rounded-3xl hover:bg-rose-50 transition-colors group border border-blue-100 shadow-sm">
                      <div className="flex items-center space-x-4">
              ...
              {activeTab === 'support' && (
                <div className="space-y-10 animate-fade-in text-left">
                  <div>
                    <h2 className="text-2xl font-black text-gray-800 tracking-tight">Priority Support</h2>
                  ...
                  <div className="space-y-6">
                    <textarea
                      ...
                      className="input-field p-8 bg-white border-blue-100 text-gray-800 placeholder-gray-400 focus:border-blue-300"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-6 bg-white rounded-3xl text-center border border-blue-100 shadow-sm">
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Live Chat</p>
                      <p className="font-bold text-gray-800">10 Min Wait</p>
                    </div>
                    <div className="p-6 bg-white rounded-3xl text-center border border-blue-100 shadow-sm">
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Email Resp</p>
                      <p className="font-bold text-gray-800">4 Hours</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'subscription' && (
                <div className="space-y-10 animate-fade-in py-10 text-center">
                  <div className="inline-flex items-center justify-center p-6 bg-emerald-50 with-border border-emerald-100 rounded-[3rem] mb-6 shadow-sm">
                    <Heart size={64} className="text-emerald-500 fill-emerald-500" />
                  </div>
                  <h2 className="text-3xl font-black text-gray-800 tracking-tight">You're on our <span className="text-emerald-500 italic">Elite</span> Plan</h2>
                  <p className="text-gray-500 max-w-sm mx-auto font-medium">Enjoy unlimited AI analysis, priority support, and real-time medical insights.</p>
                  <div className="pt-10 flex justify-center">
                    <button className="btn-secondary px-10 bg-white text-gray-700 border border-gray-200 hover:bg-gray-50">Manage Billing</button>
                  </div>
                </div>
              )}
            </div>
